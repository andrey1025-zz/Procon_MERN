...
const ModelUpload = (props) => {
    ...
    const onAddingImage = event => {
        if (event.target.files[0]) {
            ...
            if (ext !== '3dm' && ext !== 'obj' && ext !== 'zip' && ext !== 'fbx' && ext !== 'max' && ext !== 'dae' && ext !== 'rvt') {
                setErrors({ fatalError: 'Models with only .3dm, .obj, .zip, .fbx, .max are acceptable' });
                fileInputRef.current.value = "";
                return;
            }
            else if (newFile.size > 600000000) {
